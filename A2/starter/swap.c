...
/*
The swapfile functions are all implemented in this file, along with bitmap functions
to track free and used space in the swap file, and to move the virtual pages between the swapfile
and simulated physical memory. The swap_pagin and swap_pageout functions takes a frame number
and a swao offset as arguments. 
** be careful not to pass the fram field from a page table entry(pte_t) directly, since 
that would include the extra status bits. It creates a swapfile in the current directory
need to remove this file as part of the cleanup when it completes.

the rand replacement algorithm is already implemented for you, so you can test your translation
and paging replacement functionality independently of implementaing the replacement algorithms.
