 ...
 * location in physmem(This is the space for our simulated physical memory. We define a simulated page size and hence frame size of SIMPAGESIZE and allocate SIMPAGESIZE * memory size bytes for physmem) array.
 * The find_physpage() function is responsible for
...
void access_mem(char type, addr_t vaddr) {
	// find_physpace, responsible for everything to do with memory management.
...
int main(int argc, char *argv[]) {
	...
	// coremap: The coremap array represents the state of (simulated) physicla mempry. Each
	// element of the array represents a physical page frame. It records if the physical frame
	// is in use and, if so, a pointer to the page table entry for the virtual page that is using it.
